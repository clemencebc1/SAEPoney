...
class DBConnector {
    ...
    public function get_notseances_for_user(string $user){
        $sql = "SELECT DISTINCT DESCRIPTIF, DATE_SEANCE, IDSEANCE, NUMCOURS FROM SEANCE WHERE DATE_SEANCE not in (SELECT DATE_SEANCE FROM SEANCE NATURAL JOIN PARTICIPER NATURAL JOIN ADHERENT NATURAL JOIN PERSONNE WHERE EMAIL='" . $user ."' and IDADH=IDPER) and IDADH=IDPER";
        $stmt = $this->pdo->query($sql);
        $rows = $stmt->fetchAll();
        return $rows;
    }
